...
if (isset($_POST['login'])) {
    ...
    if (isset($data->success) AND $data->success == true) {
        ...
        $res = mysqli_query($dbcon, "select autostand_id,autostand_mail,autostand_password from autostand where autostand_mail='$uname' and autostand_password='$pass' ");
        $row = mysqli_fetch_array($res);

        if ($row) {

            $_SESSION["autostand_lgid"] = $row['autostand_id'];
            $_SESSION["autostand_mail"] = $row['autostand_mail'];
            

            header("location:autostand_home.php");
        } else {
            ?>
            <script>
                alert("invalid user");
...
<div class="container">
    <div class="row">
        <div class="col-sm-6 col-md-4 col-md-offset-4">
            <h1 class="text-center login-title">Sign in as AutoStand</h1>
